        ...
        if (Conexion.conectar()) {
            try {
                ...
                try (PreparedStatement ps = Conexion.getConn().prepareStatement(sql)) {
                    ...
                    while (rs.next()) {
                        tabla += "<tr>"
                               + "<td class='centrado'>" + rs.getString("id_menu") + "</td>"
                               + "<td class='centrado'>" + rs.getString("nombre_menu") + "</td>"
                               + "<td class='centrado'><input type='text' value='" + rs.getString("codigo_menu") + "' size=50 readonly='readonly'></td>"
                               + "</tr>";
                    }
                    if(tabla.equals("")){
    ...
    public static boolean agregar(Menus menu) {
        ...
        if (Conexion.conectar()) {
            String sql = "insert into menus "
                    ...
                    + "values (?,?)";
            System.out.println("sql AgregarMenu "+sql);
            try (PreparedStatement ps = Conexion.getConn().prepareStatement(sql)) {
